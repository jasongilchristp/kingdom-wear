...
const ProductGrid = () => {
  ...
  const handleAddToCart = (product, availableStock) => {
    if (availableStock <= 0) {
            ...
            return (
              <div key={product.id} className="group relative flex flex-col">
                ...
                <div className="mt-6 flex flex-col flex-1 text-center">
                  ...
                  <p className="mt-1 text-xs text-gray-400">{availableStock > 0 ? `${availableStock} in stock` : (product.stock > 0 ? 'Max in cart' : 'Sold out')}</p>
                  
                  {/* Add to cart button */}
                  <button 
                    disabled={isOutOfStock}
                    onClick={() => handleAddToCart(product, availableStock)}
                    className={`mt-5 relative z-20 w-full bg-transparent border-2 border-black py-3 px-4 rounded-sm text-xs font-bold uppercase tracking-widest transition-colors duration-300 ${isOutOfStock ? 'border-gray-300 text-gray-400 cursor-not-allowed' : 'text-black hover:bg-black hover:text-white'}`}
                  >
